...
def fourier_encode(inp, embedding_size=64, embedding_scale=12.):
    ...
    bvals = torch.from_numpy(np.reshape(np.eye(3)*bvals[:, None, None], [len(bvals)*3, 3])).float().cuda()
    avals = torch.ones_like(bvals[:, 0]).cuda()
    inp_coord = b3nTobn3(inp)
    stacked_x = torch.cat([avals * torch.sin((2. * np.pi*inp_coord) @ torch.transpose(bvals, 0, 1)),
                          avals * torch.cos((2. * np.pi*inp_coord) @ torch.transpose(bvals, 0, 1))], axis=-1) / torch.norm(avals)
    return stacked_x
